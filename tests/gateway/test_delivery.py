...
class TestCaseSensitiveChatIdParsing:
    ...
    def test_uppercase_chat_id_preserved(self):
        """Mixed-case channel IDs like C123ABC should preserve case."""
        target = DeliveryTarget.parse("discord:C123ABC")
        assert target.platform == Platform.DISCORD
        assert target.chat_id == "C123ABC"  # Should NOT be lowercased to c123abc
        assert target.is_explicit is True

    def test_chat_id_with_thread_preserved(self):
        """channel:thread IDs should preserve case."""
        target = DeliveryTarget.parse("discord:C123ABC:thread123")
        assert target.platform == Platform.DISCORD
        assert target.chat_id == "C123ABC"
        assert target.thread_id == "thread123"

    def test_room_id_with_embedded_colon_preserved(self):
        """Room IDs that themselves contain a colon (e.g. !RoomABC:example.org)
        ...
        parsing structure.
        """
        target = DeliveryTarget.parse("discord:!RoomABC:example.org")
        assert target.platform == Platform.DISCORD
