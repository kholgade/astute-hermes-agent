...
def test_wecom_open_group_with_per_group_sender_allowlist_is_authorized(monkeypatch):
    ...
    runner, _adapter = _make_runner(Platform.SLACK, config, enforces=True)

    assert runner._is_user_authorized(_source(Platform.SLACK, chat_type="group")) is True
...
def test_env_allowlist_still_takes_precedence_for_own_policy_platform(monkeypatch):
    ...
    config = GatewayConfig(
        platforms={Platform.SLACK: PlatformConfig(enabled=True, extra={"dm_policy": "open"})}
    )
    runner, _adapter = _make_runner(Platform.SLACK, config, enforces=True)

    listed = SessionSource(
        platform=Platform.SLACK, user_id="allowed-user", chat_id="c",
        user_name="t", chat_type="dm",
    )
    stranger = SessionSource(
        platform=Platform.SLACK, user_id="stranger", chat_id="c",
        user_name="t", chat_type="dm",
    )
...
def test_unknown_adapter_does_not_crash_trust_check(monkeypatch):
    ...
    assert runner._adapter_enforces_own_access_policy(Platform.SLACK) is False
    assert runner._is_user_authorized(_source(Platform.SLACK)) is False
...
def test_pairing_dm_policy_authorizes_paired_user(monkeypatch):
    ...
    _clear_auth_env(monkeypatch)
    config = GatewayConfig(
        platforms={Platform.SLACK: PlatformConfig(enabled=True, extra={"dm_policy": "pairing"})}
    )
    runner, _adapter = _make_runner(Platform.SLACK, config, enforces=True)
    runner.pairing_store.is_approved.return_value = True

    assert runner._is_user_authorized(_source(Platform.SLACK)) is True
...
def test_pairing_carveout_reads_adapter_when_env_set(monkeypatch):
    ...
    _clear_auth_env(monkeypatch)
    config = GatewayConfig(
        platforms={Platform.SLACK: PlatformConfig(enabled=True, extra={})}
    )
    runner, adapter = _make_runner(Platform.SLACK, config, enforces=True)
...
def test_unauthorized_dm_behavior_follows_config_dm_policy(monkeypatch, dm_policy, expected):
    ...
    _clear_auth_env(monkeypatch)
    config = GatewayConfig(
        platforms={Platform.SLACK: PlatformConfig(enabled=True, extra={"dm_policy": dm_policy})}
    )
    runner, _adapter = _make_runner(Platform.SLACK, config, enforces=True)

    assert runner._get_unauthorized_dm_behavior(Platform.SLACK) == expected
