...
class ToolSearchConfig:
    ...
    minimal_core: bool  # when True, only _HERMES_MINIMAL_CORE stays always-on;
    #                      every other core tool becomes deferrable (issue #17).
    ...
    def from_raw(cls, raw: Any) -> "ToolSearchConfig":
        ...
        minimal_core_raw = raw.get("minimal_core", False)
        minimal_core = str(minimal_core_raw).strip().lower() in ("true", "1", "yes", "on")
...
def _core_tool_names(config: Optional["ToolSearchConfig"] = None) -> frozenset[str]:
    ...
    Normally this is the full ``_HERMES_CORE_TOOLS`` list. When
    ``tools.tool_search.minimal_core`` is enabled, it collapses to the lean
    ``_HERMES_MINIMAL_CORE`` substrate (file + terminal + code-exec + web +
    skills loaders) so every other core tool becomes deferrable and is served
    on demand through the bridges (issue #17).

    Imported lazily because ``toolsets`` imports from ``tools.registry``
    and we don't want a hard cycle. ``config`` is loaded when not supplied;
    ``load_config`` is cheap (the underlying file read is mtime-cached).
    """
    try:
        if config is None:
            config = load_config()
        if getattr(config, "minimal_core", False):
            from toolsets import _HERMES_MINIMAL_CORE
            return frozenset(_HERMES_MINIMAL_CORE)
...
def is_deferrable_tool_name(name: str, config: Optional["ToolSearchConfig"] = None) -> bool:
    ...
    OR it is not in the always-on core set. Core tools are never deferred
    even when their toolset is technically plugin-provided (this protects
    against accidental shadowing). When ``tools.tool_search.minimal_core`` is
    enabled the always-on set shrinks to ``_HERMES_MINIMAL_CORE``, so most
    core tools become eligible (issue #17). ``config`` is resolved once by
    the caller during assembly and threaded down; standalone callers may
    omit it and pay a cheap (mtime-cached) config load.
    """
...
def classify_tools(
    ...
    every always-on core tool, plus any tool we can't classify. ``deferrable``
    is the candidate set for catalog entry. ``config`` selects the always-on
    core set (full vs ``_HERMES_MINIMAL_CORE``); resolved once here when not
    supplied so all per-tool checks agree within one assembly.
    """
    if config is None:
        config = load_config()
